...
@section('content')
    <div class="container mx-auto px-4 py-6">
        <h1 class="text-2xl font-bold mb-6">Edit Status</h1>

        <form method="POST" action="{{ route('statuses.update', $status) }}" class="space-y-4 bg-white p-6 rounded shadow">
            @csrf
            @method('PUT')
            <div>
                <label class="block font-semibold mb-1" for="name">Name</label>
                <input id="name" name="name" type="text" value="{{ old('name', $status->name) }}" class="w-full border rounded px-3 py-2" required>
                @error('name')
            ...
            <div>
                ...
                <input id="order" name="order" type="number" min="1" value="{{ old('order', $status->order) }}" class="w-full border rounded px-3 py-2" required>
                @error('order')
                    <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <select id="status_type_id" name="status_type_id" class="w-full border rounded px-3 py-2" required>
                    @foreach($statusTypes as $id => $name)
                        <option value="{{ $id }}" @selected(old('status_type_id', $status->status_type_id) == $id)>{{ $name }}</option>
                    @endforeach
                </select>
